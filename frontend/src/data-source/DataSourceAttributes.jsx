...
class DataSourceAttributes extends React.Component {
    ...
    render() {
        return (<div className="DataSourceAttributes">
            ...
            <div>
                <span className="shortLabel"> <b>Familles <QuestionCircleOutlined title="Famille fonctionnelle de la donnée" /> : </b> {this.props.dataSource.family_name.map((family_name) => (
                    <Tag color="blue">{family_name}</Tag>
                ))}</span>
            </div>
            <div>
                <span className="shortLabel"> <b>Axe d'analyse  <QuestionCircleOutlined title="Types de référentiels utilisés pour classifier la donnée" /> : </b> {this.props.dataSource.classification_name.map((classification_name) => (
                    <Tag color="purple">{classification_name}</Tag>
                ))}</span>
            </div>
            <div>
                <span className="shortLabel"> <b>Expositions <QuestionCircleOutlined title="Type de mises à disposition" /> : </b> {this.props.dataSource.exposition_name.map((tag_name) => (
                    <Tag color="gold">{tag_name}</Tag>
                ))}</span>

            </div>
            <div>
                <span className="shortLabel"> <b>Tags <QuestionCircleOutlined title="Tags de la donnée" /> : </b> {this.props.dataSource.tag_name.map((tag_name) => (
                    <Tag>{tag_name}</Tag>
                ))}</span>

            </div>
            <div className="grid">
                <span className="shortLabel one"> <b>Type <QuestionCircleOutlined title="Type de la donnée" /> : </b> <Tag color="red">{this.props.dataSource.type_name}</Tag></span>
                <span className="shortLabel two"> <b>Référentiel <QuestionCircleOutlined title="Type de référentiel s’il s’agit d’une donnée référentielle (par opposition aux données opérationnelles)" /> : </b> <Tag color="orange">{this.props.dataSource.referentiel_name}</Tag></span>
                <span className="shortLabel three"> <b>Sensibilité <QuestionCircleOutlined title="Sensibilité des données identifiantes" /> : </b> <Tag color="lime">{this.props.dataSource.sensibility_name}</Tag></span>
                <span className="shortLabel four"> <b>Open Data <QuestionCircleOutlined title="La donnée est-elle publiable en Open Data ?" /> : </b> <Tag color="green">{this.props.dataSource.open_data_name}</Tag></span>
                <span className="shortLabel five"> <b>Origine <QuestionCircleOutlined title="Origine fonctionnelle de la donnée" /> : </b> <Tag color="geekblue">{this.props.dataSource.origin_name}</Tag></span>
