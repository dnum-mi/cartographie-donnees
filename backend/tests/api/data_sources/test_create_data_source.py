...
def test_create_data_source_without_type(client, admin_auth_header, sample_applications, sample_family, sample_type):
    new_ds_with_non_existing_type = {**DEFAULT_DATA_SOURCE, 'type_name': 'type9'}
    response = client.post("/api/data-sources", headers=admin_auth_header, json=new_ds_with_non_existing_type)
    assert response.status_code == 400
    assert response.json['description'] == "La valeur 'type9' filtre type n'existe pas."


def test_create_data_source_without_family(client, admin_auth_header, sample_applications, sample_family, sample_type):
    new_ds_with_non_existing_family = {**DEFAULT_DATA_SOURCE, 'family_name': ['family9']}
    response = client.post("/api/data-sources", headers=admin_auth_header, json=new_ds_with_non_existing_family)
    assert response.status_code == 400
