...
from app.search.query_builder import create_filters_query


def test_create_filter_query_without_children(sample_organizations: List[Organization]):
    query = create_filters_query({'organization': ['MI > DGPN', 'INSEE > SG']})
    assert query == {
        "bool": {
            "should": unordered([{
                'term': {
...
def test_create_filters_query_with_children(sample_organizations: List[Organization]):
    query = create_filters_query({'organization': ['MI', 'INSEE']})
    assert query == {
        "bool": {
            "should": unordered([{
                "bool": {
